...
// Имя клиента для отображения В ПАНЕЛИ АДМИНИСТРАТОРА — если мама
// добавила свою заметку/переименование (карандаш в "Записи"), показываем
// его, иначе оригинальное имя из анкеты. Только для админки: на сайте
// клиент всегда видит только своё оригинальное client_name.
function bookingAdminDisplayName(array $booking): string
{
    $note = trim((string)($booking['admin_note'] ?? ''));
    return $note !== '' ? $note : $booking['client_name'];
}
